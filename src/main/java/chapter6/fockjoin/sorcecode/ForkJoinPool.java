...
package chapter6.fockjoin.sorcecode;
...
public class ForkJoinPool extends AbstractExecutorService {
    ...
    static final int LIFO_QUEUE   = 0; // 栈，先进后出
    static final int FIFO_QUEUE   = 1 << 16; // 队列，先进先出
    ...
    static final class WorkQueue {
         ...
         * 所以scanState的第15～0又可以表示在ForkJoinPool.workQueues数组的下标索引；
         */
        volatile int scanState;    // versioned, <0: inactive; odd:scanning
        /**
         * 当worker线程从激活变为非激活时设置值，且值为ForkJoinPool的ctl的低32位(实际是前一个非激活线程），这样就形成了一个非激活线程链；
         */
         ...
         */
        final ForkJoinTask<?>[] growArray() { // 扩容或者初始化数组
            ForkJoinTask<?>[] oldA = array;
            // 如果array已经被初始化，则扩容一倍（左移一个），否则初始化为8192
     ...
     * explanation.
     */
    // work线程每次增加的seed 2654435769 10011110001101110111100110111001
    ...
    private int awaitRunStateLock() {
        ...
        for (int spins = SPINS, r = 0, rs, ns;;) {
            ...
            else if (r == 0) // 表示当前线程池已经被获取锁(其他线程)，并且是第一次进行循环，初始化SECONDARY的数值
    ...
    private void tryAddWorker(long c) {
        boolean add = false;
        do {
            // c + AC_UNIT 表示活跃线程+1，和AC_MASK进行与操作表示只取第一个16位
            // c + TC_UNIT 表示总线程+1, 和TC_MASK进行与操作表示只取第二个16位
            long nc = ((AC_MASK & (c + AC_UNIT)) |
                       (TC_MASK & (c + TC_UNIT)));
            if (ctl == c) { // 如果ctl 等于 c表示 c没有发生变更，对ctl进行cas操作
                int rs, stop;                 // check if terminating rs为锁住后的线程池状态，用来解锁用，stop表示当前线程池状态是否是stop
                if ((stop = (rs = lockRunState()) & STOP) == 0)
                    // 等于0表示不是stop
                    // 设置ctl的值
                    add = U.compareAndSwapLong(this, CTL, c, nc);
                // 无论成功失败（cas以及是否是stop）都进行解锁
                unlockRunState(rs, rs & ~RSLOCK);
                if (stop != 0)
                    // 如果stop状态成立则直接返回
                    break;
                if (add) {
                    // 如果创建线程池数量成功，则创建worker，然后返回
                    createWorker();
                    break;
                }
            }
        } while (((c = ctl) & ADD_WORKER) != 0L && (int)c == 0);
        // 反复确认线程池数量是否已经变成最大，以及是否有空闲的线程
    }
    ...
    final WorkQueue registerWorker(ForkJoinWorkerThread wt) {
        ...
        try {
            ...
            if ((ws = workQueues) != null && (n = ws.length) > 0) {
                ...
                int m = n - 1;
                // s << 1 左移1位并且 或 1，使其变成奇数（很重要，保证工作线程在奇数上），并且保证在数组m范围之内
                i = ((s << 1) | 1) & m;               // odd-numbered indices
                // 如果ws[i]不等于null，表示发生碰撞，需要进行扩容？
                ...
                // config为i和mode的或值,i永远不会超过2个字节的值，因为workQueue的扩容跟最大线程数有关
                w.config = i | mode;
                // scanState为i；也就是索引下标
    ...
    final void signalWork(WorkQueue[] ws, WorkQueue q) {
        ...
        while ((c = ctl) < 0L) {                       // too few active  什么情况下需要进行死循环解决
            // ctl小于0表示活跃线程数还没有超过并行度
            if ((sp = (int)c) == 0) {                  // no idle workers
                // 获取低32位，如果低32位等于0，表示没有线程被阻塞
                if ((c & ADD_WORKER) != 0L)            // too few workers（没有超过最大线程池数）
                    // 活跃线程没有超过并行度并且没有正在阻塞的线程，同时也没有超过最大线程,则需要添加worker
                    tryAddWorker(c);
                break;
            }
            // 这里表示的ws位null，表示线程池还未初始化，暂时还没发现terminated处理的事情
            if (ws == null)                            // unstarted/terminated
                break; // 直接返回
            if (ws.length <= (i = sp & SMASK))         // terminated
                // 线程队列数组的大小等于线程worker所属数组的下标，理论不存在这种情况
                break;
            if ((v = ws[i]) == null)                   // terminating
                break; // 表示不存在当前阻塞的队列资源

            // 下面是将阻塞的进行唤醒，因为没有创建新的worker，使用阻塞worker来执行任务
            int vs = (sp + SS_SEQ) & ~INACTIVE;        // next scanState  新的vs值，waitcount+1并且变成激活状态
            int d = sp - v.scanState;                  // screen CAS 如果sp - v.scanState等于0表示sp和和v.scanState相等，也即是v就是当前线程池阻塞的top节点
            long nc = (UC_MASK & (c + AC_UNIT)) | (SP_MASK & v.stackPred); // 将前一个节点的scanState和activecount+1合并在一起
            ...
            // 如果唤醒工作队列失败（假设竞争资源失败，需要判断下任务队列是否还有任务，通过base和top来判断，没有任务则直接返回，也不重试去唤醒其他线程）
    ...
    final void runWorker(WorkQueue w) {
        w.growArray();                   // allocate queue 分配队列容量
        ...
        for (ForkJoinTask<?> t;;) {
            ...
            if ((t = scan(w, r)) != null)
                // 执行扫描的，并且会执行自身的
                w.runTask(t);
            // 如果没有扫描到则要进行await，没有扫描到任务，会将当前w状态设置为非激活状态
     ...
     */
    /**
     * 先扫描或窃取别人的任务，可能包括自己的，如果返回null，表示任务已经没有了，如果返回null，则表示当前线程已经变成非激活状态，并且ctl的低32位是自己
     */
                            ...
                            if (ss >= 0) { // ss大等于0表示线程是激活状态
                                if (U.compareAndSwapObject(a, i, t, null)) {
                                    ...
                                    if (n < -1)       // signal others 如果n小于-1，表示还有很多task在其中
                                        signalWork(ws, q);
                                    // 返回task（终点方法，表示找到task）
                                    return t;
                                }
                            }
                            else if (oldSum == 0 &&   // try to activate
                                     w.scanState < 0)//在判断一次线程scan状态，如果小于0则唤醒一个线程（当前线程是非激活状态了，那么需要重新唤醒一个）
    ...
    /**
     * 重点方法
     * awaitWork的前提是w的scanState是负的，表示暂时不需要这个队列去执行任务
     * @param w
     * @param r
     * @return
     */
    private boolean awaitWork(WorkQueue w, int r) {
        // w.qlock一般是在线程池关闭或者自己取消注册的时候才会变成-1，这种情况下就不需要进行await
        if (w == null || w.qlock < 0)                 // w is terminating
            return false;
        // pred是上一个stackPred
        for (int pred = w.stackPred, spins = SPINS, ss;;) {// spins初始化为0
            if ((ss = w.scanState) >= 0) // scanState大于0表示线程还在激活状态，可能是被唤醒去处理任务，理论上执行到这里一般不会是激活状态
                break;
            else if (spins > 0) {// 默认值是0（待分析）
                // 随机数混淆
                r ^= r << 6; r ^= r >>> 21; r ^= r << 7;
                ...
                return false;
            else if (!Thread.interrupted()) { // 没有被中断才执行这个，为什么要判断这个？
                // c是ctl，
                long c, prevctl, parkTime, deadline;
                // 获取实际active count线程数量（ctl高16位+线程总数）
                int ac = (int)((c = ctl) >> AC_SHIFT) + (config & SMASK);
                // ac一般不会小于0，至少等于0，判断线程池状态是否是关闭状态
                if ((ac <= 0 && tryTerminate(false, false)) ||
                    (runState & STOP) != 0)           // pool terminating
                    // 如果线程池是terminating状态，则进行返回，不对当前线程做任何处理
                    return false;
                // ac 等于0 并且ss等于 （int）c表示当前线程是最后一个waiter
                if (ac <= 0 && ss == (int)c) {        // is last waiter,最后一个不能是非激活
                    prevctl = (UC_MASK & (c + AC_UNIT)) | (SP_MASK & pred); //再增加一个线程？
                    int t = (short)(c >>> TC_SHIFT);  // shrink excess spares
                    if (t > 2 && U.compareAndSwapLong(this, CTL, c, prevctl)) // 无法理解？ 判断什么，要做什么
     ...
     */
    /**
     * 基于now和enable来判断线程池状态，以及处理线程池状态
     * @param now
     * @param enable
     * @return
     */
    private boolean tryTerminate(boolean now, boolean enable) {
        ...
        if ((rs & STOP) == 0) { // 表示当前线程还不是stop状态，需要变成stop状态
            ...
            // 执行完上面代码还没有跳出方法的，则需要立刻停止线程池（不需要等待线程池中任务执行完）（通过加锁和解锁来处理）（仅先更改状态）
        ...
        for (long oldSum = 0L;;) {                // or until done or stable
            ...
            if ((short)(checkSum >>> TC_SHIFT) + (config & SMASK) <= 0 ||
                    // (short)(checkSum >>> TC_SHIFT)表示的ctl的高32位的低16位也就是当前总线程数
                    // (config & SMASK) 表示的是线程初始化数
                    // 相加如果小于等于0表示，当前线程总数一个也没

                    // (ws = workQueues) == null || (m = ws.length - 1) <= 0表示队列还没初始化
                (ws = workQueues) == null || (m = ws.length - 1) <= 0) {
                // 如果线程和队列都没初始化，则判断线程状态是否是终止，不是的话进行终止，
            ...
            // 上面判断了当前线程和队列的运行情况，如果都没初始化则进行关闭操作
            for (int i = 0; i <= m; ++i) {
                if ((w = ws[i]) != null) {
                    checkSum += w.base;
            ...
            if (checkSum != oldSum) {             // unstable
                // 如果检测到还有线程在进行消费或者计算，则重置pass的值，归0
                oldSum = checkSum;
                pass = 0;
            }
            else if (pass > 3 && pass > m)        // can't further help
                // 如果pass大于3次并且pass的值大于m（工作队列的长度）则跳出
                break;
        ...
        for (;;) {
            ...
            if ((rs = runState) < 0) {
                ...
                throw new RejectedExecutionException();
                     ...
                     ((ws = workQueues) == null || (m = ws.length - 1) < 0)) {
                // 线程池状态不为started或者线程池队列为null或者队列长度小于0（表示线程池需要等待初始化）
                int ns = 0;
                rs = lockRunState();
                try {
                    if ((rs & STARTED) == 0) { // 对workqueue进行初始化
                        // 初始化STEALCOUNTER从null变成原子long类型
                        U.compareAndSwapObject(this, STEALCOUNTER, null,
                                               new AtomicLong());
                        // create workQueues array with size a power of two
                        int p = config & SMASK; // ensure at least 2 slots 取出低16位（这里代表线程池数，而不是队列数）
                        // 基于线程池数算出队列数量，至少是线程数量2倍
                        int n = (p > 1) ? p - 1 : 1;
                        n |= n >>> 1; n |= n >>> 2;  n |= n >>> 4;
                        n |= n >>> 8; n |= n >>> 16; n = (n + 1) << 1;
                        // 将n的所有位都变成1，最后一步n = (n + 1) << 1则是将n变成2的幂次方
            ...
            else if ((q = ws[k = r & m & SQMASK]) != null) { // 当前线程所属的工作队列已经初始化，可以进行提交
                // 获取当前线程提交的任务应该属于哪个工作队列的索引，赋值给k
                if (q.qlock == 0 && U.compareAndSwapInt(q, QLOCK, 0, 1)) { // 判断无锁并且加锁
            ...
            else if (((rs = runState) & RSLOCK) == 0) { // create new queue 当前线程所属的工作队列还未初始化，需要进行初始化
                // 初始化队列
                q = new WorkQueue(this, null);
                q.hint = r;
                // 负数，包含工作队列在workqueues的索引值
                ...
                // 将新建的workqueue放入到对应的索引中去
                unlockRunState(rs, rs & ~RSLOCK);
            }
            else
                // 表示没有机会（没有获得锁）或者没有被执行到，
    ...
    final void externalPush(ForkJoinTask<?> task) {
        ...
        if ((ws = workQueues) != null && (m = (ws.length - 1)) >= 0 &&
            ...
            if ((a = q.array) != null &&
                ...
                // top自增1位
                U.putOrderedInt(q, QTOP, s + 1);
                // 解锁（这里解锁方式和下面使用cas解锁不一样的原因在于，此处必须要写入成功，确保上面俩行更改刷新到主内存中），因为使用cas可能不会更新
                // 因为这里没有记录加锁线程，任何线程都可以解锁
        ...
        // 如果队列没有初始化，或者当前线程没有初始化（是第一次提交任务），就进行初始化队列
        externalSubmit(task);
    }
    ...
    private ForkJoinPool(int parallelism,
        ...
        this.ueh = handler;
        // 将线程数量和mode合并在一起存储(parallelism最大不超过SMASK也就是65535)
        // 低16位存储线程数量，高16位存储工作模式
    ...
    static {
        ...
        // 静态初始化forkjoinworkerthreadfactory初始化
        defaultForkJoinWorkerThreadFactory =
